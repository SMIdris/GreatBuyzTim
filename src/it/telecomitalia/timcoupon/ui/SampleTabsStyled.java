...
import com.onmobile.utils.GBWebViewClient;
...
public class SampleTabsStyled extends FragmentActivity implements OnCategoryClick, OnDealItemClick, OnPageChangeListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// TIM Special
		String name = Utils.getMessageString(AppConstants.Messages.msgStrSpecial, R.string.resStrSpecial);
		CONTENT.clear();
		CONTENT.add(0, name);

		// Deals of The Day
		name = Utils.getMessageString(AppConstants.Messages.dealsOfTheDay, R.string.dealsOfTheDay);
		CONTENT.add(1, name);
		...
		CONTENT.add(3, name);
		categoryListFragmentPosition = 3;
		categoryListTabName = name;

		// Deals near me
		name = Utils.getMessageString(AppConstants.Messages.dealsNearMe, R.string.dealsNearMe);
		CONTENT.add(4, name);
		...
		// Deals You May Like
		/*
		 * name = getResources().getString(R.string.dealsYouMayLike); srvName =
		 * _data.getMessage(AppConstants.Messages.dealsYouMayLike);
		 * if(!Utils.isNothing(srvName)) name = srvName; CONTENT[7] = name;
		 */
	...
	public void showWebView(String title, String url, boolean checkRefresh)
	{
		...
		if (!titleView.getText().equals(mwebTitle) || checkRefresh)
		{
			...
			if (webViewClient == null)
				webViewClient = new GBWebViewClient(SampleTabsStyled.this);
	...
	private void populateSettingsMenu()
	{
		...
		mDrawerLayout.setDrawerListener(new DrawerListener()
		{
			...
			public void onDrawerOpened(View arg0)
			{
				...
				{
					if (mAdapter.mSurpriseMeFragment != null)
						((SurpriseMeFragment) mAdapter.mSurpriseMeFragment).stopSensor();
				}
			...
			public void onDrawerClosed(View arg0)
			{
				...
				{
					if (mAdapter.mSurpriseMeFragment != null)
						((SurpriseMeFragment) mAdapter.mSurpriseMeFragment).startSensor();
				}
	...
	private void createSettingsSubView(int id)
	{
		...
		switch (id)
		{
			...
			case AppConstants.SettingItems.INFO:
				...
				/*
				 * String version = Utils.getClientVersion();
				 * if(!Utils.isNothing(version)) settingSubListItems.add(new
				 * SettingItem(AppConstants.SettingItems.VERSION, version,
				 * null));
				 */
				break;
			// case AppConstants.SettingItems.HELP:
			// if (settingSubListItems != null &&
			// !settingSubListItems.isEmpty())
			// settingSubListItems.clear();
			//
			// settingSubListItems = new ArrayList<SettingItem>();
			//
			// settingSubListItems.add(new
			// SettingItem(AppConstants.SettingItems.FAQ,
			// getResources().getString(R.string.menuFAQ), this
			// .getResources().getDrawable(R.drawable.setting_faq)));
			// break;
			default:
				break;
		}
	...
	private void createSettingsView()
	{
		...
		settingItems.add(new SettingItem(AppConstants.SettingItems.NOTIFICATIONS, getResources().getString(
				R.string.settingItemNotifications), SampleTabsStyled.this.getResources().getDrawable(R.drawable.setting_faq)));
		...
		/*
		 * settingItems.add(new SettingItem(AppConstants.SettingItems.KEYWORDS,
		 * getResources().getString(R.string.settingItemKeywords),
		 * getApplicationContext
		 * ().getResources().getDrawable(R.drawable.setting_help)));
		 */
		// settingItems.add(new SettingItem(AppConstants.SettingItems.ABOUT,
		// getResources().getString(R.string.menuAbout), this.getResources()
		// .getDrawable(R.drawable.setting_alert)));
		//
		// settingItems.add(new SettingItem(AppConstants.SettingItems.HELP,
		// getResources().getString(R.string.menuHelp), this.getResources()
		// .getDrawable(R.drawable.setting_help)));
	...
	private void stopAllDownload()
	{
		...
		try
		{
			if (mAdapter != null)
			{
				...
				{
					if (((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).listDownloader != null)
						((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).listDownloader.cancel(true);
					if (((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).adapter != null)
						((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).adapter.reset();
				}
				if (((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment) != null)
				{
					if (((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).listDownloader != null)
						((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).listDownloader.cancel(true);
					if (((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).adapter != null)
						((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).adapter.reset();
				}
				if (mAdapter.mFragmentForCategory != null && mAdapter.mFragmentForCategory instanceof DealsByCategoryFragment)
				{
					if (((DealsByCategoryFragment) mAdapter.mFragmentForCategory).listDownloader != null)
						((DealsByCategoryFragment) mAdapter.mFragmentForCategory).listDownloader.cancel(true);
					if (((DealsByCategoryFragment) mAdapter.mFragmentForCategory).adapter != null)
						((DealsByCategoryFragment) mAdapter.mFragmentForCategory).adapter.reset();
				}
				if (((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment) != null)
				{
					if (((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).listDownloader != null)
						((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).listDownloader.cancel(true);
					if (((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).adapter != null)
						((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).adapter.reset();
				}
				if (((DealsNearMeFragment) mAdapter.mDealsNearMeFragment) != null)
				{
					if (((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).listDownloader != null)
						((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).listDownloader.cancel(true);
					if (((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).adapter != null)
						((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).adapter.reset();
				}
				if (mAdapter.mFragmentForExploreCategory instanceof ExploreDealResultFragment
						&& mAdapter.mFragmentForExploreCategory != null)
				{
					if (((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).listDownloader != null)
						((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).listDownloader.cancel(true);
					if (((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).adapter != null)
						((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).adapter.reset();
				}
				/*if (((MyDealFragment) mAdapter.mMyDealsFragment) != null)
				{
					if (((MyDealFragment) mAdapter.mMyDealsFragment).listDownloader != null)
						((MyDealFragment) mAdapter.mMyDealsFragment).listDownloader.cancel(true);
					if (((MyDealFragment) mAdapter.mMyDealsFragment).adapter != null)
						((MyDealFragment) mAdapter.mMyDealsFragment).adapter.reset();
				}*/
				if (((MyDealsFragment) mAdapter.mMyDealsFragment) != null)
				{
					if (((MyDealsFragment) mAdapter.mMyDealsFragment).listDownloader != null)
						((MyDealsFragment) mAdapter.mMyDealsFragment).listDownloader.cancel(true);
					if (((MyDealsFragment) mAdapter.mMyDealsFragment).adapter != null)
						((MyDealsFragment) mAdapter.mMyDealsFragment).adapter.reset();
				}
			}
		}
		catch (Exception e)
	...
	public void onBack()
	{
		...
		else if (isCategoryAfterExplorDisplayed && mPager.getCurrentItem() == mSearchIndex - 1 && isSearchVisible)
		{
	...
	/*private void exit()
	{
		...
		// ////System.out.println("GreatBuyz: function IN SampleTabsStyled > exit");
		stopAllDownload();
		...
		try
		{
			if (mAdapter != null)
			{
				if (((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment) != null)
				{
					((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).listDownloader = null;
					((ExclusiveDealsFragment) mAdapter.mExclusiveDealsFragment).adapter = null;
				}
				if (((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment) != null)
				{
					((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).listDownloader = null;
					((DealsOfTheDayFragment) mAdapter.mDealsOfTheDayFragment).adapter = null;
				}
				if (mAdapter.mFragmentForCategory != null && mAdapter.mFragmentForCategory instanceof DealsByCategoryFragment)
				{
					((DealsByCategoryFragment) mAdapter.mFragmentForCategory).listDownloader = null;
					((DealsByCategoryFragment) mAdapter.mFragmentForCategory).adapter = null;
				}
				if (((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment) != null)
				{
					((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).listDownloader = null;
					((DealsYouMayLikeFragment) mAdapter.mDealsYouMayLikeFragment).adapter = null;
				}
				if (((DealsNearMeFragment) mAdapter.mDealsNearMeFragment) != null)
				{
					((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).listDownloader = null;
					((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).adapter = null;
				}
				if (mAdapter.mFragmentForExploreCategory instanceof ExploreDealResultFragment
						&& mAdapter.mFragmentForExploreCategory != null)
				{
					((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).listDownloader = null;
					((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).adapter = null;
				}
				if (((MyDealFragment) mAdapter.mMyDealsFragment) != null)
				{
					((MyDealFragment) mAdapter.mMyDealsFragment).listDownloader = null;
					((MyDealFragment) mAdapter.mMyDealsFragment).adapter = null;
				}
				if (((MyDealsFragment) mAdapter.mMyDealsFragment) != null)
				{
					((MyDealsFragment) mAdapter.mMyDealsFragment).listDownloader = null;
					((MyDealsFragment) mAdapter.mMyDealsFragment).adapter = null;
				}
			}
		}
		catch (Exception e)
		...
		// ////System.out.println("GreatBuyz: function OUT SampleTabsStyled > exit");
	}*/
	...
	{
		// ////System.out.println("GreatBuyz: function IN SampleTabsStyled > onSaveInstanceState");

		// ////System.out.println("GreatBuyz: ST  onSaveInstanceState start");
		...
		// ////System.out.println("GreatBuyz: function OUT SampleTabsStyled > onSaveInstanceState");
	}
	...
	protected void onRestoreInstanceState(Bundle savedInstanceState)
	{
		...
		// ////System.out.println("GreatBuyz: function IN SampleTabsStyled > onRestoreInstanceState");

		// ////System.out.println("GreatBuyz: ST  onRestoreInstanceState start");
		...
		// ////System.out.println("GreatBuyz: function OUT SampleTabsStyled > onRestoreInstanceState");
	}

	@Override
	protected void onDestroy()
	{
		// //System.out.println("GreatBuyz: function IN SampleTabsStyled > onDestroy");
		super.onDestroy();
		mAdapter = null;
		// ////System.out.println("GreatBuyz: function OUT SampleTabsStyled > onDestroy");
	}
	...
	class GoogleMusicAdapter extends FragmentStatePagerAdapter
	{
		...
		public Fragment getItem(int position)
		{
			...
			switch (position + 1)
			{
				...
				case AppConstants.FramentConstants.CATEGORY_FRAGMENT:
					...
					{
						mFragmentForCategory = CategoryListFragment.newInstance("Hemant3", SampleTabsStyled.this, SampleTabsStyled.this,
								categoryFragmentNavigationListner);
					}
					mFragmentForCategory.setRetainInstance(true);
					return mFragmentForCategory;
				case AppConstants.FramentConstants.DEALS_NEAR_ME:
					mDealsNearMeFragment = DealsNearMeFragment.newInstance("Hemant4", SampleTabsStyled.this, SampleTabsStyled.this);
					mDealsNearMeFragment.setRetainInstance(true);
					return mDealsNearMeFragment;
				...
				case AppConstants.FramentConstants.EXPLORE_DEALS:
					if (mFragmentForExploreCategory == null)
						mFragmentForExploreCategory = ExploreDealsFragment.newInstance("Hemant6", SampleTabsStyled.this,
								exploreFragmentNavigationListner);
				...
				/*case AppConstants.FramentConstants.MY_DEALS:
					mMyDealsFragment = MyDealFragment.newInstance("Hemant7", SampleTabsStyled.this, SampleTabsStyled.this);
		...
		public int getItemPosition(Object object)
		{
			if (object instanceof CategoryListFragment && mFragmentForCategory instanceof DealsByCategoryFragment)
				...
				return POSITION_NONE;
			else if (object instanceof ExploreDealResultFragment && mFragmentForExploreCategory instanceof ExploreDealsFragment)
				return POSITION_NONE;
	...
	protected Dialog onCreateDialog(int id, Bundle args)
	{
		...
		switch (id)
		{
			...
			case AppConstants.DialogConstants.IN_APP_EXIT_DIALOG:
			{
				if (args.containsKey(AppConstants.JSONKeys.MESSAGE))
					message = args.getString(AppConstants.JSONKeys.MESSAGE);
				...
				btn.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						try
						{
							removeOtherDialog(AppConstants.DialogConstants.IN_APP_EXIT_DIALOG);
							exitFromApp();
						}
						catch (Exception e)
						{
			...
			case AppConstants.DialogConstants.EMAIL_DIALOG:
				{
					...
					emailDialog.setButton(Dialog.BUTTON_NEUTRAL, strOk, new DialogInterface.OnClickListener()
					{
						...
						public void onClick(DialogInterface dialog, int which)
						{
							...
							if (modifiedEmailValue != null && emailId != null && !modifiedEmailValue.equals(AppConstants.EMPTY_STRING)
									&& modifiedEmailValue.equals(emailId))
							{
								closeMenu();
								return;
							}
							if (modifiedEmailValue == null)
								modifiedEmailValue = AppConstants.EMPTY_STRING;
			...
			runOnUiThread(new Runnable()
			{
				public void run()
				{
					try
					{
						...
						mAdapter.mFragmentForCategory = CategoryListFragment.newInstance("Hemant2", SampleTabsStyled.this,
								SampleTabsStyled.this, categoryFragmentNavigationListner);
						mAdapter.notifyDataSetChanged();
					}
					catch (Exception e)
					{
			...
			runOnUiThread(new Runnable()
			{
				public void run()
				{
					try
					{
						...
						mAdapter.mFragmentForCategory = DealsByCategoryFragment.newInstance(category, SampleTabsStyled.this,
								categoryFragmentNavigationListner);
						mAdapter.notifyDataSetChanged();
					}
					catch (Exception e)
	...
	ExplorePageFragmentListener exploreFragmentNavigationListner = new ExplorePageFragmentListener()
	{
		...
		public void onSwitchToPrevFragment()
		{
			...
			mAdapter.mFragmentForExploreCategory = ExploreDealsFragment.newInstance("", SampleTabsStyled.this,
					exploreFragmentNavigationListner);
			mAdapter.notifyDataSetChanged();
		}
		...
		public void onSwitchToNextFragment(String category, String city, String locality, String keyword)
		{
			...
			mAdapter.mFragmentForExploreCategory = ExploreDealResultFragment.newInstance(SampleTabsStyled.this, keyword, category, city,
					locality, SampleTabsStyled.this);
			mAdapter.notifyDataSetChanged();
		}
	};
	...
	public void setUserPreferencesAndUpdateScreen(final String selectedCity)
	{
		try
		{
			...
			GreatBuyzApplication.getServiceDelegate().updateUserInfo(new String[] { selectedCity }, null, null, null, null, -2, null, null,
					null,null, new IOperationListener()
					{
						@Override
	...
	private void getFragmentToUpdate(int position)
	{
		...
		switch (position + 1)
		{
			...
			case AppConstants.FramentConstants.DEALS_YOU_MAY_LIKE:
				...
				break;
			case AppConstants.FramentConstants.DEALS_NEAR_ME:
				((DealsNearMeFragment) mAdapter.mDealsNearMeFragment).refreshFragment();
				mAdapter.notifyDataSetChanged();
				break;

			/*case AppConstants.FramentConstants.MY_DEALS:
				((MyDealFragment) mAdapter.mMyDealsFragment).refreshFragment();
				mAdapter.notifyDataSetChanged();
				break;*/
			case AppConstants.FramentConstants.EXPLORE_DEALS:
			case AppConstants.FramentConstants.MY_DEALS:
				...
				if (((MyDealsFragment) mAdapter.mMyDealsFragment) != null)
					((MyDealsFragment) mAdapter.mMyDealsFragment).refreshFragment();

				/* for AppConstants.FramentConstants.EXPLORE_DEALS */
				if (isCategoryAfterExplorDisplayed)
				{
					if (((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory) != null)
						((ExploreDealResultFragment) mAdapter.mFragmentForExploreCategory).refreshFragment();
				}
				else
				{
					if (((ExploreDealsFragment) mAdapter.mFragmentForExploreCategory) != null)
						((ExploreDealsFragment) mAdapter.mFragmentForExploreCategory).refreshFragment();
				}
	...
	public void onPageSelected(int index)
	{
		...
		// hide back button
		if ((index + 1 == AppConstants.FramentConstants.EXPLORE_DEALS && isCategoryAfterExplorDisplayed)
				|| (index + 1 == AppConstants.FramentConstants.CATEGORY_FRAGMENT && mAdapter.mFragmentForCategory instanceof DealsByCategoryFragment))
		{
			imgBack.setVisibility(View.VISIBLE);
			mBackNavigation.setClickable(true);
		}
	...
	public void setUserPreferencesEmailId(final String emailId)
	{
		try
		{
			...
			showDialog(AppConstants.DialogConstants.LOADING_DIALOG, null);
			GreatBuyzApplication.getServiceDelegate().updateUserInfo(null, null, emailId, null, null, -2, null, null, null,null,
					new IOperationListener()
					{
						@Override
						public void onOperationCompleted(boolean pOperationComplitionStatus, String pMessageFromServer)
			...
			GreatBuyzApplication.getServiceDelegate().unsubscribeFromChannel(mdn, channel, chargingMode, new IOperationListener()
			{
				...
				public void onOperationCompleted(boolean pOperationComplitionStatus, String pMessageFromServer)
				{
					...
					if (pOperationComplitionStatus)
					{
						...
						//_app.saveStringPreference(AppConstants.SharedPrefKeys.isClientUnsubScribe, "DISCONTINUED");
						_data.updateUserSubscriptionStatus("DISCONTINUED");
	...
	private void selectChildItem(int position)
	{
		...
		switch (id)
		{
			...
			case AppConstants.SettingItems.INFO_PRIVACY:
				closeMenu();
				String INFO_PRIVACY_URL = GreatBuyzApplication.getApplication().getINFOPRIVACYURL();				
				showWebView(Utils.getMessageString(AppConstants.Messages.infoPrivacyTitle, R.string.infoPrivacyTitle), INFO_PRIVACY_URL, false);
				showWebView("", "", true);
				break;
			case AppConstants.SettingItems.INFO_COSTI:
				closeMenu();
				String INFO_COSTI_URL = GreatBuyzApplication.getApplication().getINFOCOSTIURL();
				showWebView(Utils.getMessageString(AppConstants.Messages.infoCostiTitle, R.string.infoCostiTitle), INFO_COSTI_URL, false);
				showWebView("", "", true);
				break;
	...
	public void openSearchTab()
	{
		...
		mIndicator.notifyDataSetChanged();
		if (isCategoryAfterExplorDisplayed && isSearchVisible)
			exploreFragmentNavigationListner.onSwitchToPrevFragment();
	}
