...
import it.telecomitalia.timcoupon.data.Contact;
import it.telecomitalia.timcoupon.data.Coupon;
import it.telecomitalia.timcoupon.data.Deal;
import it.telecomitalia.timcoupon.data.Location;
import it.telecomitalia.timcoupon.data.Merchant;
import it.telecomitalia.timcoupon.data.NotificationDTO;
import it.telecomitalia.timcoupon.data.PhoneLine;
import it.telecomitalia.timcoupon.data.Purchase;
import it.telecomitalia.timcoupon.data.TnC;
import it.telecomitalia.timcoupon.ui.WelcomeBonusActivity;
...
public class ResponseParser
{
	...
	public static JSONObject parseGetInfo(JSONObject resultObject) throws JSONException
	{
		...
		String oldVersion = _data.getVersion(DB.COL_VERSION_WELCOME_DEAL);
		newVersion = versions.optString(AppConstants.JSONKeys.WELCOMEDEAL);
		if (!Utils.isNothing(newVersion))
		{
			if(!oldVersion.equalsIgnoreCase(newVersion) && userObject != null)
			
			_data.updateVersion(DB.COL_VERSION_WELCOME_DEAL, newVersion);
			SharedPreferences.Editor editor = GreatBuyzApplication
					.getApplication().getSharedPreferences().edit();
			editor.putBoolean(DB.COL_VERSION_WELCOME_DEAL, true);
			editor.commit();
			
			versions.remove(AppConstants.JSONKeys.WELCOMEDEAL);
		}
	...
	public static void parseVersions(JSONArray versionsArray, JSONObject values) throws JSONException
	{
		...
		prevVersion = _data.getVersion(DB.COL_VERSION_WELCOME_DEAL);
		newVersion = newVersions.get(AppConstants.JSONKeys.WELCOMEDEAL);
		if (!newVersion.equalsIgnoreCase(prevVersion))
			values.put(AppConstants.JSONKeys.WELCOMEDEAL, newVersion);
	}
	...
	public static Merchant parseMerchant(JSONObject merchantObject) throws JSONException
	{
		String name = merchantObject.optString(AppConstants.JSONKeys.NAME, null);
		JSONObject contactObject = merchantObject.optJSONObject(AppConstants.JSONKeys.CONTACT);
	...
	public static Deal parseDeal(JSONObject dealObject) throws JSONException
	{
		...
		try
		{
			...
			if(jsonLocation.has("district")){
				locationDistrict = jsonLocation.getString("district");	
				System.out.println("locationDistrict ***"+locationDistrict);
			}
		...
		if (tnc == null)
		{
			if (endDate != null)
			{
				tnc = new TnC(null, endDate, 0, 0, null, null);
			}
			else
			{
		...
		/*
		 * Merchant merchant = null; try { merchant =
		 * parseMerchant(dealObject.optJSONObject
		 * (AppConstants.JSONKeys.MERCHANT)); } catch (Exception e) {
		 * e.printStackTrace(); }
		 * 
		 * if (merchant == null) { String merchantName =
		 * dealObject.optString(AppConstants.JSONKeys.MERCHANT, null); merchant
		 * = new Merchant(merchantName, null); }
		 * 
		 * List<String> operators = null; JSONArray operatorsArray =
		 * dealObject.optJSONArray(AppConstants.JSONKeys.OPERATORS); if
		 * (operatorsArray != null && operatorsArray.length() > 0) { operators =
		 * new ArrayList<String>(); for (int i = 0; i < operatorsArray.length();
		 * i++) { String operator = (String) operatorsArray.opt(i);
		...
		return new Deal(id, null, 0, name, null, longDescription, image, couponPrice, price, discount, null, location, locationDistrict, null, null, tnc, null,
	...
	{
		long purchaseTimeStampMillis = purchaseObject.optLong(AppConstants.JSONKeys.PURCHASE_TIMESTAMP, -1);
		Date purchaseTime = null;
		if (purchaseTimeStampMillis != -1)
			purchaseTime = new Date(purchaseTimeStampMillis);
