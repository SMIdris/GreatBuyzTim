...
public class SampleTabsStyled extends FragmentActivity implements OnCategoryClick, OnDealItemClick, OnPageChangeListener
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		// }
		// });
		ImageView imgHeaderIcon = (ImageView) findViewById(R.id.imgHeaderIcon);
		imgHeaderIcon.setOnTouchListener(new View.OnTouchListener()
		{
			@Override
				...
				if (event.getAction() == MotionEvent.ACTION_DOWN)
				{
					...
					handler.postDelayed(new Runnable()
					{
						...
						public void run()
						{
							...
							{
								// set your code here
								// Toast.makeText(getApplicationContext(), "isLongPress imgHeaderIcon", Toast.LENGTH_LONG).show();
								...
								Bundle b = new Bundle();
								showDialog(AppConstants.DialogConstants.ENABLE_LOGS_DIALOG, b);
							}
						}
					}, longClickDuration);
				}
	...
	protected Dialog onCreateDialog(int id, Bundle args)
	{
		...
		switch (id)
		{
			...
			case AppConstants.DialogConstants.ENABLE_LOGS_DIALOG:
			{
				final GenericDialog dialog = new GenericDialog(SampleTabsStyled.this, R.layout.enable_logs_message, R.style.AlertDialogCustom);
				dialog.setCancelable(false);
				final ToggleButton tgl = (ToggleButton) dialog.findViewById(R.id.logsToggleButton);
				final TextView tglName = (TextView) dialog.findViewById(R.id.enableLogs);
				SharedPreferences pref = GreatBuyzApplication.getApplication().getSharedPreferences();
				boolean isLogsEnabled = pref.getBoolean(AppConstants.SharedPrefKeys.enableLogs, false);
				tgl.setChecked(isLogsEnabled);
				if (isLogsEnabled)
					tglName.setText("Disable Logs");
				else
					tglName.setText("Enable Logs");
				tgl.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener()
				{
					...
					public void onCheckedChanged(CompoundButton buttonView, boolean isChecked)
					{
						if (isChecked)
						{
							...
							// sched.setAlarm(true);
							// SharedPreferences pref = GreatBuyzApplication.getApplication().getSharedPreferences();
							// boolean isNewWelcomeDeal = pref.getBoolean(AppConstants.SharedPrefKeys.enableLogs, false);
//							SharedPreferences pref = GreatBuyzApplication.getApplication().getSharedPreferences();
//							Editor prefEdit = pref.edit();
//							prefEdit.putBoolean(AppConstants.SharedPrefKeys.enableLogs, isChecked);
//							prefEdit.commit();
						}
						else
						{
...
//							prefEdit.commit();
				...
				btn.setOnClickListener(new OnClickListener()
				{
					...
					public void onClick(View v)
					{
						try
						{
							SharedPreferences pref = GreatBuyzApplication.getApplication().getSharedPreferences();
							Editor prefEdit = pref.edit();
							prefEdit.putBoolean(AppConstants.SharedPrefKeys.enableLogs, tgl.isChecked());
							prefEdit.commit();
							removeOtherDialog(AppConstants.DialogConstants.ENABLE_LOGS_DIALOG);
							// exitFromApp();
				...
				TextView msgText = (TextView) dialog.findViewById(R.id.msg);
				msgText.setText("You must restart the application to apply these changes");
				msgText.setTypeface(_app.getFont());
				TextView titleText1 = (TextView) dialog.findViewById(R.id.title);
				titleText1.setText(AppConstants.EMPTY_STRING);
				titleText1.setTypeface(_app.getFont());
