...
public final class DealsByCategoryFragment extends Fragment
{
	...
	public void onActivityCreated(Bundle savedInstanceState)
	{
		...
		if (GreatBuyzApplication.getDataController().getDealsByCategoriesDTO() != null
				&& GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals().size() > 0)
		{
			initialiseList();
		}
		else
		...
		lsComposer.setOnItemClickListener(new OnItemClickListener()
		{
			...
			{
				if (arg2 < GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals().size())
					Utils.startDetailsScreenNew(getActivity(), arg2, AppConstants.FramentConstants.DEAL_BY_CATEGORY);
			}
	...
	class MyListDownloader extends AsyncTask<Void, Void, ClipResponse>
	{
		...
		protected ClipResponse doInBackground(Void... params)
		{
			try
			{
				ClipResponse clipResponse = GreatBuyzApplication.getServiceDelegate().getDealsByCategories(AppConstants.JSONKeys.WAP,
						selectedCategory, GreatBuyzApplication.getApplication().getLimitForDealByCategory(),
						GreatBuyzApplication.getApplication().getSkipIndexForDealByCategory());
				return clipResponse;
			}
			catch (Exception e)
			{
	...
	public boolean morePageAvailable()
	{
		...
		try
		{
			if (GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals().size() >= totalVisibleItems
					&& currentCount >= GreatBuyzApplication.getApplication().getLimitForDealByCategory())
				morePage = true;
		}
		catch (Exception e)
		{
	...
	public void refreshFragment()
	{
		...
		listDownloader = null;
		if (GreatBuyzApplication.getDataController().getDealsByCategoriesDTO() != null
				&& GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals() != null
				&& GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals().size() > 0)
		{
			GreatBuyzApplication.getDataController().getDealsByCategoriesDTO().getDeals().clear();
		}
